...
% Moss & Ross 2011 BSSA procedure for Average Displacement
...
for m = 1 : 1 : 251 %this gives the mag bins from 5-7
    ...
    for r = 1 : 1 : 51 %this gives the x/L bins from 0 to 0.5 with 
        ...
        %A is the Prob(AD|M) from linear regression (compared to Wells and
        %Coppersmith regression) 0.17 * 2.302 is the standard deviation
        %term in ln space
        
        %B is the Prob(D/AD|M,r) from statistical fitting of gamma 

        A = trlnrnd(mu((5+(2/250) * (m - 1))) , 0.17*2.302, 5000);
        
        B = gamrnd(a_gam(0.5*(r - 1)/50), b_gam(0.5*(r - 1)/50), 1, 5000);
